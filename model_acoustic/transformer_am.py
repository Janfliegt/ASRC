...
class Transformer_encoder(tf.keras.Model):
    def __init__(self, num_heads, num_blocks, d_model, d_ff, pinyin_size, dropout_rate):
        super().__init__()
        ...
        self.Dense_layer_out = tf.keras.layers.Dense(pinyin_size, activation='softmax')

    def call(self, input, input_length, input_length_decode):
        """
        :param xs:
        :param training:
        :return:
        ...
        k = 8
        batch_size, time_frames, feat_dim = input.shape[0], input.shape[1], input.shape[2]
        input_reformed = tf.reshape(input, [batch_size, time_frames // k, feat_dim * k])
        input_length = input_length // k
        input_length_decode = input_length_decode // k

        enc = self.dense_in(input_reformed)
        enc = enc + positional_encoding(enc, seq_len)
        ...
        return logits, input_length, input_length_decode